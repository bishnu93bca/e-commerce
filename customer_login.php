...
<form method="post" action="">
	...
	<tr>
		...
		<td align="left"><input type="text" name="email" placeholder="Entewr email"/></td>
...
if(isset($_POST['login']))
{
	...
	//$run_c=$con->quert($sel_c);
	$check_customer=mysqli_num_rows($run_c);

	if($check_customer==0){
		echo "<script>alert('Password or Email incorrect and try agin')</script>";
		exit();
	}
	$ip=getIp();
	...
	if($check_customer>0 AND $check_cart==1){
		...
		echo "<script>alert('You logged in successful,Thanks!')</script>";
		echo "<script>window.open('customer/my_account.php','_self')</script>";
	}else{
		$_SESSION['customer_email']=$c_email;
		echo "<script>alert('logged in successful,Thanks!')</script>";
